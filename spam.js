...
const customerList = customers.map(customer => `${customer.first_name} ${customer.last_name}`)
...
const customerEmailList = customers.map(customerEmail => {


    return  customerEmail = customerEmail.contacts.email.filter(singleEmail =>
       `${singleEmail}`)
